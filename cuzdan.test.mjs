...
// Testler duvar saatine bağlı olmamalı. "Şimdi" olarak bugünün öğlesi
// alınıyor: üstüne bir iki saat eklemek gün sınırını geçmiyor. SIMDI
// kullanıldığında test gece 23:00'ten sonra çalışınca "aynı gün" varsayımları
// çöküyordu — kod değil, testin saat seçimi yanlıştı.
const SIMDI = (function () {
  const d = new Date();
  d.setHours(12, 0, 0, 0);
  return d.getTime();
})();
...
t('10 alışveriş → net toplamın %5\'i bakiye', () => {
  ...
  let son;
  for (let i = 0; i < 10; i++) {
    son = P.alisverisEkle(s, { brutKurus: 30000, ts: SIMDI - (10 - i) * GUN });  // 300 TL × 10 = 3.000 TL
  }
...
t('işletmenin üç örneği birebir tutuyor', () => {
  for (const [toplam, beklenen] of [[3000, 150], [5000, 250], [10000, 500]]) {
    const s = yeniUye();
    let son;
    for (let i = 0; i < 10; i++) son = P.alisverisEkle(s, { brutKurus: P.kurus(toplam / 10), ts: SIMDI - (10 - i) * GUN });
...
t('alt sınırın altındaki gün alışveriş sayılmaz', () => {
  const s = yeniUye();
  const r = P.alisverisEkle(s, { brutKurus: 5000, ts: SIMDI });   // 50 TL < 75 TL
  assert.equal(r.sayildi, false);
  assert.equal(P.turDurumu(s).alisveris, 0);
  // aynı gün ek alışverişle sınır aşılınca sayılır
  const r2 = P.alisverisEkle(s, { brutKurus: 3000, ts: SIMDI + 1000 });  // toplam 80 TL
...
t('bakiye tavanı brüt tutarın %25\'i', () => {
  const s = yeniUye();
  for (let i = 0; i < 10; i++) P.alisverisEkle(s, { brutKurus: 100000, ts: SIMDI - (10 - i) * GUN }); // 10.000 TL
...
t('bakiyeyle ödenen kısım yeni harcamaya sayılmaz', () => {
  const s = yeniUye();
  for (let i = 0; i < 10; i++) P.alisverisEkle(s, { brutKurus: 100000, ts: SIMDI - (20 - i) * GUN });
  assert.equal(P.aktifBakiye(s), 50000);
  const r = P.alisverisEkle(s, { brutKurus: 200000, bakiyeKurus: 50000, ts: SIMDI });
...
t('önce en yakın sona erecek parti düşer', () => {
  const s = yeniUye();
  P.bakiyeVer(s, 10000, 'ig', SIMDI - 25 * GUN);   // 5 gün ömrü kaldı
  P.bakiyeVer(s, 10000, 'ig2', SIMDI - GUN);       // 29 gün ömrü var
  P.alisverisEkle(s, { brutKurus: 100000, bakiyeKurus: 15000, ts: SIMDI });
...
t('tahmini bakiye müşteriye doğru sayıyı gösterir', () => {
  const s = yeniUye();
  for (let i = 0; i < 4; i++) P.alisverisEkle(s, { brutKurus: 25000, ts: SIMDI - (4 - i) * GUN });
